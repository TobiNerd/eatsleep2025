Shader "Hidden/Locked/.poiyomi/Poiyomi Toon Outline Early/d1e39a62317bb694c8440efe7f551452"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			)
			{
				...
				else if ((1.0 /*_OutlineExpansionMode*/) == 3)
				{
					...
					offsetMultiplier = saturate(dot(viewNormal.xy, normalize(float4(0,0,0,0).xy)));
					offset *= offsetMultiplier;
					offset *= distanceOffset;
				}
				else if ((1.0 /*_OutlineExpansionMode*/) == 4)
				{
					offset = mul((float3x3)transpose(UNITY_MATRIX_V), float4(0,0,0,0));
					offset *= distanceOffset;
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.7799658,0.7213061,0.4939446,1).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.7799658,0.7213061,0.4939446,1).a;
