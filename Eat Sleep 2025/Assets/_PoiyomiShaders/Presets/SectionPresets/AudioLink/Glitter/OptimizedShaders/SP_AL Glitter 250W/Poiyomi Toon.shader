Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/bab679e29a5dc0842b989b06e3c58b18"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				for (uint glitterLayer = 0; glitterLayer < uint((2.0 /*_GlitterLayers*/)); glitterLayer++)
				{
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						size = saturate(size + lerp(float4(0,0.4,0,0).x, float4(0,0.4,0,0).y, poiMods.audioLink[(0.0 /*_GlitterALSizeAddBand*/)]));
					}
					#endif
					half glitterAlpha = 1;
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						glitterSpeedOffset += AudioLinkGetChronoTime((0.0 /*_GlitterALChronoSparkleSpeedType*/), (0.0 /*_GlitterALChronoSparkleSpeedBand*/)) * (0.2 /*_GlitterALChronoSparkleSpeed*/);
					}
					#endif
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						glitterMask = clamp(glitterMask + lerp(float4(0,0.9,0,0).x, float4(0,0.9,0,0).y, poiMods.audioLink[(1.0 /*_GlitterALAlphaAddBand*/)]), 0, glitterMask);
					}
					#endif
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						GlitterbrightnessOffset = max(GlitterbrightnessOffset +lerp(float4(0,100,0,0).x, float4(0,100,0,0).y, poiMods.audioLink[(2.0 /*_GlitterALMaxBrightnessBand*/)]), 0);
					}
					#endif
