Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/6da4ec4deb4ed3d4088216c8ce0aba67"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				for (uint glitterLayer = 0; glitterLayer < uint((2.0 /*_GlitterLayers*/)); glitterLayer++)
				{
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						size = saturate(size + lerp(float4(0,0.5,0,0).x, float4(0,0.5,0,0).y, poiMods.audioLink[(0.0 /*_GlitterALSizeAddBand*/)]));
					}
					#endif
					half glitterAlpha = 1;
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						glitterSpeedOffset += AudioLinkGetChronoTime((0.0 /*_GlitterALChronoSparkleSpeedType*/), (0.0 /*_GlitterALChronoSparkleSpeedBand*/)) * (0.3 /*_GlitterALChronoSparkleSpeed*/);
					}
					#endif
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						glitterMask = clamp(glitterMask + lerp(float4(0,1,0,0).x, float4(0,1,0,0).y, poiMods.audioLink[(1.0 /*_GlitterALAlphaAddBand*/)]), 0, glitterMask);
					}
					#endif
					...
					if ((1.0 /*_GlitterALEnabled*/))
					{
						GlitterbrightnessOffset = max(GlitterbrightnessOffset +lerp(float4(0,800,0,0).x, float4(0,800,0,0).y, poiMods.audioLink[(2.0 /*_GlitterALMaxBrightnessBand*/)]), 0);
					}
					#endif
