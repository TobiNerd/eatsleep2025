Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/1e9adbdc238838e468e43866c87229b1"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float CalculateAmbientOcclusion(float2 uv, float depth, float3 normal, float radius, float4 worldDirection, PoiMesh poiMesh, PoiCam poiCam)
			{
				...
				for (int sampleIndex = 0; sampleIndex < totalSamples; sampleIndex++)
				{
					...
					aoValue *= smoothstep(max(radius + float4(5,5,1,1).x + EPSILON, radius + float4(5,5,1,1).y), radius + float4(5,5,1,1).x, depthDifference);
					ao += aoValue;
				}
			...
			float calculateSSAO(PoiMesh poiMesh, PoiCam poiCam, inout PoiLight poiLight, inout PoiMods poiMods)
			{
				...
				float ssao = CalculateAmbientOcclusion(screenPos, depth, transformedNormal, (0.005 /*_SSAORadius*/), direction, poiMesh, poiCam);
			...
			void applySSAO(float ssao, in PoiMesh poiMesh, inout PoiFragData poiFragData, inout PoiMods poiMods, inout PoiLight poiLight)
			{
				...
				float3 ssaoColor = poiThemeColor(poiMods, float4(0.2086369,0.05126944,0.08650043,1).rgb, (0.0 /*_SSAOColorThemeIndex*/)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_SSAOUseSurfaceColor*/));
