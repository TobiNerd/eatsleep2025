Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/373cd0213376b804cb8cd79eadd924d0"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			CGPROGRAM
 #define POI_SSAO 
 #define SSAO_COLOR_TEX 
 #define VIGNETTE_MASKED 
 #define _LIGHTINGMODE_FLAT 
 #define _STOCHASTICMODE_DELIOT_HEITZ 
 #define PROP_SSAOCOLORMAP 
 #define OPTIMIZER_ENABLED 
			...
			float CalculateAmbientOcclusion(float2 uv, float depth, float3 normal, float radius, float4 worldDirection, PoiMesh poiMesh, PoiCam poiCam)
			{
				...
				int totalSamples = int(((3.0 /*_SSAOQuality*/) * 5) * smoothstep((8.0 /*_SSAOFalloffEnd*/), (6.0 /*_SSAOFalloffStart*/), poiCam.clipPos.w));
				...
				float totalWeight = 0;
				float randomValue = (0.333 /*_SSAORandomScale*/) * (SSAOInterleavedGradientNoise(uv * _ScreenParams.xy) * 2.0 - 1.0);
				[loop]
				for (int sampleIndex = 0; sampleIndex < totalSamples; sampleIndex++)
				{
					...
					float depthDifference = depth - sampleDepth - (0.0001 /*_SSAOBias*/);
					float weight = 1;
					aoValue = saturate(depthDifference / radius);
					aoValue *= smoothstep(max(radius + float4(0,0.1,1,1).x + EPSILON, radius + float4(0,0.1,1,1).y), radius + float4(0,0.1,1,1).x, depthDifference);
					ao += aoValue;
				}
			...
			float calculateSSAO(PoiMesh poiMesh, PoiCam poiCam, inout PoiLight poiLight, inout PoiMods poiMods)
			{
				...
				float ssao = CalculateAmbientOcclusion(screenPos, depth, transformedNormal, (0.00289 /*_SSAORadius*/), direction, poiMesh, poiCam);
			...
			void applySSAO(float ssao, in PoiMesh poiMesh, inout PoiFragData poiFragData, inout PoiMods poiMods, inout PoiLight poiLight)
			{
				...
				float3 ssaoColor = poiThemeColor(poiMods, float4(1,1,1,1).rgb, (0.0 /*_SSAOColorThemeIndex*/)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_SSAOUseSurfaceColor*/));
				#ifdef SSAO_COLOR_TEX
				#if defined(PROP_SSAOCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				float2 uv = (1.0 /*_SSAOAsRamp*/) ? ssao : poiMesh.uv[(0.0 /*_SSAOColorMapUV*/)] ;
				ssaoColor *= POI2D_SAMPLER_PAN(_SSAOColorMap, _MainTex, poiUV(uv, float4(1,1,0,0)), float4(3,0,0,0));
				#endif
				#endif
				if (any(float4(0,0,0,2)))
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, lilToneCorrection(poiFragData.baseColor, float4(0,0,0,2) + 1), (1 - ssao) * (3.0 /*_SSAOIntensity*/));
				}
				poiFragData.baseColor = lerp(poiFragData.baseColor, customBlend(poiFragData.baseColor, ssaoColor, (2.0 /*_SSAOBlendType*/)), (1 - ssao) * (3.0 /*_SSAOIntensity*/));
			}
			#endif
