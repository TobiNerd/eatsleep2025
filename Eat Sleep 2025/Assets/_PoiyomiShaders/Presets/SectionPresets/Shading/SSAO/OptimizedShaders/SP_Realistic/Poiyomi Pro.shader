Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/901169f46aee6f1499541dd3f1e80db6"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float CalculateAmbientOcclusion(float2 uv, float depth, float3 normal, float radius, float4 worldDirection, PoiMesh poiMesh, PoiCam poiCam)
			{
				...
				for (int sampleIndex = 0; sampleIndex < totalSamples; sampleIndex++)
				{
					...
					float depthDifference = depth - sampleDepth - (0.001 /*_SSAOBias*/);
					float weight = 1;
			...
			void applySSAO(float ssao, in PoiMesh poiMesh, inout PoiFragData poiFragData, inout PoiMods poiMods, inout PoiLight poiLight)
			{
				...
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, lilToneCorrection(poiFragData.baseColor, float4(0,0,0,0) + 1), (1 - ssao) * (1.333 /*_SSAOIntensity*/));
				}
				poiFragData.baseColor = lerp(poiFragData.baseColor, customBlend(poiFragData.baseColor, ssaoColor, (2.0 /*_SSAOBlendType*/)), (1 - ssao) * (1.333 /*_SSAOIntensity*/));
			}
			#endif
