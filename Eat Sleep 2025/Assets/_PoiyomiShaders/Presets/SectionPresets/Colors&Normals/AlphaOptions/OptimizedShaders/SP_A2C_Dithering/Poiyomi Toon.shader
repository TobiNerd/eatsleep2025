Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/11cc49b1463fe4240ae384b23baf0f67"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDithering(inout PoiFragData poiFragData, in PoiCam poiCam)
			{
				...
				{
					float dither = calcDither(poiCam.posScreenPixels) - (0.0 /*_AlphaDitherBias*/);
					poiFragData.alpha = saturate(poiFragData.alpha - (dither * (1 - poiFragData.alpha) * (1.0 /*_AlphaDitherGradient*/)));
				}
			}
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if ((1.0 /*_Mode*/) == 1)
				{
					...
					if ((0.0 /*_AlphaSharpenedA2C*/) && (1.0 /*_AlphaToCoverage*/))
					{
						poiFragData.alpha *= 1 + max(0, CalcMipLevel(poiMesh.uv[0] * float4(1,1,1,1).zw)) * (0.25 /*_AlphaMipScale*/);
						poiFragData.alpha = (poiFragData.alpha - (0.01 /*_Cutoff*/)) / max(fwidth(poiFragData.alpha), 0.0001) + (0.01 /*_Cutoff*/);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if ((1.0 /*_Mode*/) == POI_MODE_CUTOUT && !(1.0 /*_AlphaToCoverage*/))
				{
					poiFragData.alpha = 1;
				}
