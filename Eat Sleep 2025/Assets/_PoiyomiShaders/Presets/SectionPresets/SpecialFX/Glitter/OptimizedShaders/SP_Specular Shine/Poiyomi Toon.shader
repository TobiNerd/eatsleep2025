Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/b7ff8e1d4597a6e4e97d7a44575217eb"
{
	Properties
	{
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				for (uint glitterLayer = 0; glitterLayer < uint((2.0 /*_GlitterLayers*/)); glitterLayer++)
				{
					...
					switch((0.0 /*_GlitterMode*/))
					{
						...
						if ((15.0 /*_GlitterSpeed*/) + glitterSpeedOffset > 0)
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, (15.0 /*_GlitterAngleRange*/), (15.0 /*_GlitterSpeed*/), glitterSpeedOffset);
						}
						else
						{
							randomRotation = randomFloat3Range(randoPoint, (15.0 /*_GlitterAngleRange*/));
						}
						float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, (0.5 /*_GlitterBias*/)), poiCam.viewDir)), (225.0 /*_GlitterContrast*/)), 0);
						finalGlitter *= glitterAlpha;
						break;
						case 1:
						float randomOffset = random(randoPoint);
						float brightness = (sin((_Time.x * 10 + randomOffset +glitterSpeedOffset) * (15.0 /*_GlitterSpeed*/)) * .5 + .5);
						...
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(specAngle, (225.0 /*_GlitterContrast*/)), 0);
					...
					glitterMask *= lerp(1, poiLight.rampedLightMap, (0.96 /*_GlitterHideInShadow*/));
					glitterMask *= lerp(1, poiLight.directLuminance, (0.96 /*_GlitterScaleWithLighting*/));
