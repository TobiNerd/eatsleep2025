Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/a78201e1ed7109a409bf47c1bdc344ae"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 RandomColorFromPoint(float2 rando, PoiMods poiMods)
			{
				...
				fixed saturation = lerp(float4(0.1999996,0.3999996,0,1).x, float4(0.1999996,0.3999996,0,1).y, rando.x);
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				for (uint glitterLayer = 0; glitterLayer < uint((2.0 /*_GlitterLayers*/)); glitterLayer++)
				{
					...
					half3 glitterColor = poiThemeColor(poiMods, float4(8,8,8,1).rgb, (0.0 /*_GlitterColorThemeIndex*/));
					...
					switch((0.0 /*_GlitterMode*/))
					{
						...
						if ((5.0 /*_GlitterSpeed*/) + glitterSpeedOffset > 0)
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, (30.0 /*_GlitterAngleRange*/), (5.0 /*_GlitterSpeed*/), glitterSpeedOffset);
						}
						else
						{
							randomRotation = randomFloat3Range(randoPoint, (30.0 /*_GlitterAngleRange*/));
						}
						float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, (0.8 /*_GlitterBias*/)), poiCam.viewDir)), (800.0 /*_GlitterContrast*/)), 0);
						finalGlitter *= glitterAlpha;
						break;
						case 1:
						float randomOffset = random(randoPoint);
						float brightness = (sin((_Time.x * 10 + randomOffset +glitterSpeedOffset) * (5.0 /*_GlitterSpeed*/)) * .5 + .5);
						...
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(specAngle, (800.0 /*_GlitterContrast*/)), 0);
					...
					glitterMask *= lerp(1, poiLight.rampedLightMap, (0.5 /*_GlitterHideInShadow*/));
					glitterMask *= lerp(1, poiLight.directLuminance, (0.5 /*_GlitterScaleWithLighting*/));
					glitterMask *= float4(8,8,8,1).a;
					...
					{
						glitterColor.rgb = hueShift(glitterColor.rgb, (0.0 /*_GlitterHueShift*/) + _Time.x * (0.0 /*_GlitterHueShiftSpeed*/), (0.0 /*_GlitterHueShiftColorSpace*/), (1.0 /*_GlitterHueSelectOrShift*/));
					}
					...
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * ((3.7 /*_GlitterBrightness*/) + GlitterbrightnessOffset), finalGlitter * glitterTexture.a * glitterMask);
						poiFragData.emission += finalGlitter * glitterColor * max(0, (((3.7 /*_GlitterBrightness*/) + GlitterbrightnessOffset) - 1) * glitterTexture.a) * glitterMask;
					}
					else
					{
						poiFragData.emission += finalGlitter * glitterColor * ((3.7 /*_GlitterBrightness*/) + GlitterbrightnessOffset) * glitterTexture.a * glitterMask;
					}
				}
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.03550518,0.03550518,0.03550518,1).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.03550518,0.03550518,0.03550518,1).a;
