Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/fd19370832eeea842a5b49505a5c4f25"
{
	Properties
	{
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				for (uint glitterLayer = 0; glitterLayer < uint((2.0 /*_GlitterLayers*/)); glitterLayer++)
				{
					...
					switch((0.0 /*_GlitterMode*/))
					{
						...
						if ((3.0 /*_GlitterSpeed*/) + glitterSpeedOffset > 0)
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, (63.0 /*_GlitterAngleRange*/), (3.0 /*_GlitterSpeed*/), glitterSpeedOffset);
						}
						else
						{
							randomRotation = randomFloat3Range(randoPoint, (63.0 /*_GlitterAngleRange*/));
						}
						float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, (0.2 /*_GlitterBias*/)), poiCam.viewDir)), (282.0 /*_GlitterContrast*/)), 0);
						finalGlitter *= glitterAlpha;
						break;
						case 1:
						float randomOffset = random(randoPoint);
						float brightness = (sin((_Time.x * 10 + randomOffset +glitterSpeedOffset) * (3.0 /*_GlitterSpeed*/)) * .5 + .5);
						...
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(specAngle, (282.0 /*_GlitterContrast*/)), 0);
					...
					glitterMask *= lerp(1, poiLight.rampedLightMap, (0.8 /*_GlitterHideInShadow*/));
					glitterMask *= lerp(1, poiLight.directLuminance, (0.8 /*_GlitterScaleWithLighting*/));
					...
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * ((1.0 /*_GlitterBrightness*/) + GlitterbrightnessOffset), finalGlitter * glitterTexture.a * glitterMask);
						poiFragData.emission += finalGlitter * glitterColor * max(0, (((1.0 /*_GlitterBrightness*/) + GlitterbrightnessOffset) - 1) * glitterTexture.a) * glitterMask;
					}
					else
					{
						poiFragData.emission += finalGlitter * glitterColor * ((1.0 /*_GlitterBrightness*/) + GlitterbrightnessOffset) * glitterTexture.a * glitterMask;
					}
				}
			}
			#endif
