Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/3ab4e086078cb2b48aa5d21f3983b7c6"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					float2 st = (poiMesh.uv[(0.0 /*_GlitterUV*/)] + float4(0,0.2,0,0).xy * _Time.x) * (60.0 /*_GlitterFrequency*/);
					...
					float size = (0.152 /*_GlitterSize*/);
					
					if ((1.0 /*_GlitterRandomSize*/))
					{
						size = lerp(float4(0,0.2810126,0,1).x, float4(0,0.2810126,0,1).y, randomFromPoint);
					}
					#ifdef POI_AUDIOLINK
					if ((0.0 /*_GlitterALEnabled*/))
					{
					...
					switch((1.0 /*_GlitterShape*/))
					{
						...
						if ((1.0 /*_GlitterRandomRotation*/) == 1 || (0.0 /*_GlitterTextureRotation*/) != 0 || float4(0,8,0,0).x != 0 || float4(0,8,0,0).y != 0 || glitterRotationTimeOffset != 0)
						{
							float2 center = float2(0, 0);
							float2 glitterRandomRotationSpeed = 0;
							float randomBoy = 0;
							
							if ((1.0 /*_GlitterRandomRotation*/) || float4(0,8,0,0).x != 0 || float4(0,8,0,0).y != 0)
							{
								randomBoy = random(m_point * 200);
								glitterRandomRotationSpeed = lerp(float4(0,8,0,0).x, float4(0,8,0,0).y, randomBoy);
							}
					...
					switch((0.0 /*_GlitterMode*/))
					{
						...
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, (20.0 /*_GlitterAngleRange*/), (10.0 /*_GlitterSpeed*/), glitterSpeedOffset);
						}
						else
						{
							randomRotation = randomFloat3Range(randoPoint, (20.0 /*_GlitterAngleRange*/));
						}
						float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, (0.8 /*_GlitterBias*/)), poiCam.viewDir)), (36.0 /*_GlitterContrast*/)), 0);
						...
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(specAngle, (36.0 /*_GlitterContrast*/)), 0);
					...
					if ((1.0 /*_GlitterRandomRotation*/) == 1 || (0.0 /*_GlitterTextureRotation*/) != 0 || float4(0,8,0,0).x != 0 || float4(0,8,0,0).y && !(1.0 /*_GlitterShape*/) || glitterRotationTimeOffset != 0)
					{
					...
					{
						glitterColor.rgb = hueShift(glitterColor.rgb, (0.0 /*_GlitterHueShift*/) + _Time.x * (2.0 /*_GlitterHueShiftSpeed*/), (0.0 /*_GlitterHueShiftColorSpace*/), (1.0 /*_GlitterHueSelectOrShift*/));
					}
					...
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * ((4.0 /*_GlitterBrightness*/) + GlitterbrightnessOffset), finalGlitter * glitterTexture.a * glitterMask);
						poiFragData.emission += finalGlitter * glitterColor * max(0, (((4.0 /*_GlitterBrightness*/) + GlitterbrightnessOffset) - 1) * glitterTexture.a) * glitterMask;
					}
					else
					{
						poiFragData.emission += finalGlitter * glitterColor * ((4.0 /*_GlitterBrightness*/) + GlitterbrightnessOffset) * glitterTexture.a * glitterMask;
					}
				}
			}
			#endif
