Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/33f8d183175c68244ad2fa34d946b2c9"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				#else
				dissolveToTexture = float4(poiThemeColor(poiMods, float4(1,1,1,0).rgb, (0.0 /*_DissolveTextureColorThemeIndex*/)), float4(1,1,1,0).a);
				#endif
				#if defined(PROP_DISSOLVENOISETEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				+ (0.0 /*_DissolveAlpha8*/)
				+ (0.0 /*_DissolveAlpha9*/);
				float dds = (0.15 /*_DissolveDetailStrength*/);
				...
				if ((1.0 /*_EnableDissolveAudioLink*/) && poiMods.audioLinkAvailable)
				{
				...
				#if defined(PROP_DISSOLVEDETAILNOISE) || !defined(OPTIMIZER_ENABLED)
				float dissolveDetailNoise = POI2D_SAMPLER_PAN(_DissolveDetailNoise, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DissolveDetailNoiseUV*/)], float4(12,12,0,0)), float4(0,0,0,0));
				#else
				...
				if ((1.0 /*_DissolveInvertDetailNoise*/))
				{
					dissolveDetailNoise = 1 - dissolveDetailNoise;
				}
				if ((40.0 /*_ContinuousDissolve*/) != 0)
				{
					da = sin(_Time.x * (40.0 /*_ContinuousDissolve*/)) * .5 + .5;
				}
				da *= dissolveMask;
				dissolveAlpha = da;
				edgeAlpha = 0;
				...
				switch((1.0 /*_DissolveType*/))
				{
					...
					{
						da = remap(da, 0, 1, -(0.25 /*_DissolveEdgeWidth*/), 1);
						dissolveAlpha = da;
						...
						edgeAlpha = remapClamped(da + (0.25 /*_DissolveEdgeWidth*/), da, noise) * (1 - dissolveAlpha);
						break;
					}
					...
					case 3: // Spherical
					{
						...
						{
							da = remap(da, 1, 0, -(0.25 /*_DissolveEdgeWidth*/), 1);
						}
						else
						{
							da = remap(da, 0, 1, -(0.25 /*_DissolveEdgeWidth*/), 1);
						...
						if ((0.0 /*_SphericalDissolveInvert*/))
						{
							...
							edgeAlpha = smoothstep((0.25 /*_DissolveEdgeWidth*/) + .00001, 0, -normalizedDistance);
						}
						else
						{
							dissolveAlpha = (normalizedDistance < 0) ? 1 : 0;
				...
				if ((0.25 /*_DissolveEdgeWidth*/) || ((1.0 /*_DissolveType*/) == 2 && (0.1 /*_DissolveP2PEdgeLength*/) != 0))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(1,0.2140411,0.401978,1).rgb, (0.0 /*_DissolveEdgeColorThemeIndex*/)), float4(1,0.2140411,0.401978,1).a);
					#ifndef POI_SHADOW
				...
				poiFragData.emission += lerp(0, dissolveToTexture * (0.0 /*_DissolveToEmissionStrength*/), dissolveAlpha) + lerp(0, edgeColor.rgb * (2.0 /*_DissolveEdgeEmission*/), smoothstep(0, 1 - (0.0 /*_DissolveEdgeHardness*/) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				clip(poiFragData.alpha - (0.01 /*_Cutoff*/));
