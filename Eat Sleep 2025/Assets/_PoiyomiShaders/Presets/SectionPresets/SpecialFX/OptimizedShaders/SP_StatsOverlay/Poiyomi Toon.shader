Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/a3665acad4265154d91456049f7e958e"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyPositionText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(0,0.13,0,0), (0.0 /*_TextPositionRotation*/), float4(0.4,0.035,1,1), uv);
				...
				fixed4 textPositionPadding = float4(0.1,0.15,0.6,0.25);
				...
				globalTextEmission += poiThemeColor(poiMods, float4(1,0,1,1).rgb, (0.0 /*_TextPositionColorThemeIndex*/)) * opacity * (0.8 /*_TextPositionEmissionStrength*/);
			}
			void ApplyTimeText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(0,0.08,0,0), (0.0 /*_TextTimeRotation*/), float4(0.4,0.06,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				fixed4 textTimePadding = float4(0.03,0.095,0.245,0.107);
				...
				sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
				float opacity = clamp(sigDist + 0.5, 0, 1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, poiThemeColor(poiMods, float4(0,0.005828385,1,1).rgb, (0.0 /*_TextTimeColorThemeIndex*/)), opacity * float4(0,0.005828385,1,1).a);
				globalTextEmission += poiThemeColor(poiMods, float4(0,0.005828385,1,1).rgb, (0.0 /*_TextTimeColorThemeIndex*/)) * opacity * (0.7 /*_TextTimeEmissionStrength*/);
			}
			void ApplyFPSText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				...
				uv = TransformUV(float4(0,0,0,0), (0.0 /*_TextFPSRotation*/), float4(0.4,0.1,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				float startUV = 1 / totalCharacters * currentCharacter;
				float endUV = 1 / totalCharacters * (currentCharacter + 1);
				float4 textFPSPadding = float4(0.02,0.08,0.19,0.08);
				...
				sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
				float opacity = clamp(sigDist + 0.5, 0, 1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, poiThemeColor(poiMods, float4(0,1,0.9787213,1).rgb, (0.0 /*_TextFPSColorThemeIndex*/)), opacity * float4(0,1,0.9787213,1).a);
				globalTextEmission += poiThemeColor(poiMods, float4(0,1,0.9787213,1).rgb, (0.0 /*_TextFPSColorThemeIndex*/)) * opacity * (2.0 /*_TextFPSEmissionStrength*/);
			}
			void ApplyNumericText(inout PoiFragData poiFragData, float2 uv, in PoiMods poiMods)
			{
				if ((3.0 /*_TextNumericWholeDigits*/) == 0 && (2.0 /*_TextNumericDecimalDigits*/) == 0)
				{
					return;
				}
				uint wholeNumber = 0;
				uint decimalNumber = 0;
				uint wholeDigits = (3.0 /*_TextNumericWholeDigits*/);
				...
				float charSign = (420.69 /*_TextNumericValue*/) >= 0 ? ASCII_SPACE : ASCII_NEGATIVE;
				NumericArray[arrayIndex] = charSign;						//First character is always the sign
				arrayIndex++;
				if (wholeDigits > 0)
				{
					wholeNumber = uint(glsl_mod(abs((420.69 /*_TextNumericValue*/)), pow(10, wholeDigits)));
					...
					while (arrayIndex <= wholeDigits)
					{
						...
						if ((1.0 /*_TextNumericTrimZeroes*/) == true)
						{
				...
				if (decimalDigits > 0)
				{
					...
					arrayIndex++;
					totalCharacters++;
					decimalNumber = uint(frac(abs((420.69 /*_TextNumericValue*/))) * pow(10.00001, decimalDigits));    // Isolate the decimal number
				...
				uv = TransformUV(float4(-0.02,-0.095,0,0), (0.0 /*_TextNumericRotation*/), float4(0.4,0.1,1,1), uv);
				if (uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
				{
					return;
				}
				...
				float startUV = 1 / totalCharacters * currentCharacter;
				float endUV = 1 / totalCharacters * (currentCharacter + 1);
				float4 textNumericPadding = float4(0.01,0.06,0.14,0.06);
				...
				sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
				float opacity = clamp(sigDist + 0.5, 0, 1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, poiThemeColor(poiMods, float4(0.6920712,1,0,1).rgb, (0.0 /*_TextNumericColorThemeIndex*/)), opacity * float4(0.6920712,1,0,1).a);
				globalTextEmission += poiThemeColor(poiMods, float4(0.6920712,1,0,1).rgb, (0.0 /*_TextNumericColorThemeIndex*/)) * opacity * (1.9 /*_TextNumericEmissionStrength*/);
			}
