Shader "Hidden/Locked/.poiyomi/Poiyomi Toon Grab Pass/28d009a006c1c5a43b0c5bc26fb204aa"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 grabpassBlur(float2 uv, float4 grabPos, float smoothness)
			{
				...
				float2 radius = ((0.355 /*_GrabBlurDistance*/) + smoothness) * .1;
				radius.x *= aspect; // Adjust for aspect ratio
				float quality = floor((5.0 /*_GrabBlurQuality*/));
				float directions = floor((8.0 /*_GrabBlurDirections*/));
			...
			inline float4 Refraction(float indexOfRefraction, float chromaticAberration, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, inout PoiLight poiLight, in PoiFragData poiFragData)
			{
				...
				else
				{
					...
					refractionColor = grabpassBlur(refractedGrab, poiCam.posScreenSpace, (1 - poiFragData.smoothness) * (1.0 /*_GrabBlurUseSmoothness*/));
					#else
			...
			void applyGrabEffects(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					grabpassColor = Refraction((1.544 /*_RefractionIndex*/), (0.0 /*_RefractionChromaticAberattion*/), poiMesh, poiCam, poiMods, poiLight, poiFragData).rgb;
				}
				else
				{
					#ifdef CHROMATIC_ABERRATION_LOW
					grabpassColor = grabpassBlur(poiCam.screenUV, poiCam.posScreenSpace, (1 - poiFragData.smoothness) * (1.0 /*_GrabBlurUseSmoothness*/));
					#else
				...
				#ifdef POI_PASS_ADD
				poiFragData.baseColor = poiBlend((5.0 /*_GrabSrcBlend*/), float4(poiFragData.baseColor, poiFragData.alpha), (3.0 /*_GrabDstBlend*/), float4(0, 0, 0, 1), blendFactor);
				#endif
				#ifdef POI_PASS_BASE
				poiFragData.baseColor = poiBlend((5.0 /*_GrabSrcBlend*/), float4(poiFragData.baseColor, poiFragData.alpha), (3.0 /*_GrabDstBlend*/), float4(grabpassColor / poiLight.finalLighting, 1), blendFactor);
				#endif
			}
			...
			{
				float smoothness = (0.965 /*_MochieRoughnessMultiplier*/);
				...
				#endif
				reflectionMask *= (0.6 /*_MochieReflectionStrength*/);
				specularMask *= (0.6 /*_MochieSpecularStrength*/);
			...
			{
				float clearCoatMask = (0.5 /*_ClearCoatStrength*/);
				float smoothness = (0.975 /*_ClearCoatSmoothness*/);
				float reflectionMask = (0.6 /*_ClearCoatReflectionStrength*/);
				float specularMask = (0.6 /*_ClearCoatSpecularStrength*/);
				#if defined(PROP_CLEARCOATMAPS) || !defined(OPTIMIZER_ENABLED)
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if ((3.0 /*_LightingDirectionMode*/) == 0)
				{
				...
				if ((3.0 /*_LightingDirectionMode*/) == 1 || (3.0 /*_LightingDirectionMode*/) == 2)
				{
					if ((3.0 /*_LightingDirectionMode*/) == 1)
					{
						poiLight.direction = mul(unity_ObjectToWorld, float4(0,0,0,1)).xyz;;
					}
					if ((3.0 /*_LightingDirectionMode*/) == 2)
					{
				...
				if ((3.0 /*_LightingDirectionMode*/) == 3) // UTS
				{
					float3 defaultLightDirection = normalize(UNITY_MATRIX_V[2].xyz + UNITY_MATRIX_V[1].xyz);
					float3 lightDirection = normalize(lerp(defaultLightDirection, _WorldSpaceLightPos0.xyz, any(_WorldSpaceLightPos0.xyz)));
					poiLight.direction = lightDirection;
				}
				if ((3.0 /*_LightingDirectionMode*/) == 4) // OpenLit
				{
				...
				if ((3.0 /*_LightingDirectionMode*/) == 5) // View Direction
				{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.5209957,0.3249489,0.3005438,0).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.5209957,0.3249489,0.3005438,0).a;
