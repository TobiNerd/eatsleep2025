Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/4439ffc714f4ad44ab0bd637620826d7"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			)
			{
				...
				if ((1.0 /*_AudioLinkAnimToggle*/))
				{
					if (AudioLinkIsAvailable())
					{
						lineWidth += lerp(float4(0,0.05,0,0).x, float4(0,0.05,0,0).y, AudioLinkData(uint2(0, (0.0 /*_AudioLinkOutlineSizeBand*/))));
					}
				}
				#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.1890294,0.3467151,0.6231937,1).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.1890294,0.3467151,0.6231937,1).a;
				#ifdef COLOR_GRADING_HDR
				#if defined(PROP_MAINCOLORADJUSTTEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				if ((1.0 /*_MainHueShiftToggle*/) == 1)
				{
					...
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, hueShift(poiFragData.baseColor, shift + (4.0 /*_MainHueShiftSpeed*/) * _Time.x, (0.0 /*_MainHueShiftColorSpace*/), (1.0 /*_MainHueShiftSelectOrShift*/)), hueShiftAlpha.r);
					}
					else
					{
						poiFragData.baseColor = hueShift(poiFragData.baseColor, frac((shift - (1 - hueShiftAlpha.r) + (4.0 /*_MainHueShiftSpeed*/) * _Time.x)), (0.0 /*_MainHueShiftColorSpace*/), (1.0 /*_MainHueShiftSelectOrShift*/));
					}
				}
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				...
				if ((1.0 /*_OutlineClipAtZeroWidth*/))
				{
					...
					if (poiMods.audioLinkAvailable)
					{
						lineWidth += lerp(float4(0,0.05,0,0).x, float4(0,0.05,0,0).y, poiMods.audioLink[(0.0 /*_AudioLinkOutlineSizeBand*/)]);
					}
					#endif
				...
				poiFragData.baseColor = lerp(col, col * poiFragData.baseColor * lerp(1, poiFragData.baseColor, (1.0 /*_PoiUTSStyleOutlineBlend*/)), (1.0 /*_OutlineTintMix*/));
				...
				if ((0.0 /*_OutlineHueShift*/))
				{
					...
					hsvg.r += (3.0 /*_OutlineHueOffsetSpeed*/) * _Time.x;
				...
				if (poiMods.audioLinkAvailable)
				{
					emission += lerp(float4(0,1,0,0).x, float4(0,1,0,0).y, poiMods.audioLink[(2.0 /*_AudioLinkOutlineEmissionBand*/)]);
				}
				#endif
