Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/7a50a7b05ade5824585c295d7b29f4fe"
{
	Properties
	{
 ...
 #define PROP_DISTORTIONFLOWTEXTURE1 
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_DISTORTIONFLOWTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector = POI2D_SAMPLER_PAN(_DistortionFlowTexture, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DistortionFlowTextureUV*/)], float4(3,3,0,0)), float4(0,0,0,0)) * 2 - 1;
				#else
				float4 flowVector = -1;
				#endif
				#if defined(PROP_DISTORTIONFLOWTEXTURE1) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector1 = POI2D_SAMPLER_PAN(_DistortionFlowTexture1, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DistortionFlowTexture1UV*/)], float4(3,3,0,0)), float4(0,0,0,0)) * 2 - 1;
				#else
				...
				#endif
				half distortionStrength = (0.035 /*_DistortionStrength*/);
				half distortionStrength1 = (0.05 /*_DistortionStrength1*/);
				#ifdef POI_AUDIOLINK
				...
				return poiMesh.uv[(0.0 /*_DistortionUvToDistort*/)] + ((flowVector.xy + flowVector1.xy) / 2) * distortionMask;
			}
			#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[(7.0 /*_MainTexUV*/)].xy, float4(3,3,0,0));
