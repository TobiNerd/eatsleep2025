Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/2be46401421f6214296d9c0d90df8745"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyBackFaceColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				{
					float4 backFaceColor = float4(0.05087607,0.1160161,0.2140411,0.5019608);
			...
			float CalculateAmbientOcclusion(float2 uv, float depth, float3 normal, float radius, float4 worldDirection, PoiMesh poiMesh, PoiCam poiCam)
			{
				...
				for (int sampleIndex = 0; sampleIndex < totalSamples; sampleIndex++)
				{
					...
					sampleDir = normalize(lerp(sampleDir, normal, (0.333 /*_SSAOCone*/)));
			...
			void applySSAO(float ssao, in PoiMesh poiMesh, inout PoiFragData poiFragData, inout PoiMods poiMods, inout PoiLight poiLight)
			{
				...
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, lilToneCorrection(poiFragData.baseColor, float4(0,0,0,1) + 1), (1 - ssao) * (1.0 /*_SSAOIntensity*/));
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.attenuationStrength = (0.3 /*_LightingCastedShadows*/);
