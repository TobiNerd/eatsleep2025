Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/c3bc736fe193e3f43a32e9f2ab399887"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyBackFaceColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				{
					float4 backFaceColor = float4(0.1720796,0.1720796,0.2158605,0.7490196);
			...
			{
				float3 x0, x1, x2;
				float3 conv = lerp(GreenWrapSH(wrap), GreenWrapSHOpt(wrap), (1.0 /*_LightingWrappedNormalization*/)); // Should try optimizing this...
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if ((1.0 /*_LightingAdditiveType*/) == 1)
				{
					...
					#endif
					float2 ToonAddGradient = float2((0.2 /*_LightingAdditiveGradientStart*/), (0.4 /*_LightingAdditiveGradientEnd*/));
				...
				float3 directColor = _LightColor0.rgb * saturate(RTWrapFunc(poiLight.nDotL, (1.5 /*_LightingWrappedWrap*/), (1.0 /*_LightingWrappedNormalization*/)));
				float3 indirectColor = 0;
				#ifdef UNITY_PASS_FORWARDBASE
				indirectColor = ShadeSH9_wrapped(lerp(poiMesh.normals[0], poiMesh.normals[1], (1.0 /*_LightingIndirectUsesNormals*/)), (1.5 /*_LightingWrappedWrap*/)) * poiLight.occlusion;
				#endif
				directColor = lerp(directColor, dot(directColor, float3(0.299, 0.587, 0.114)), (0.0 /*_LightingMonochromatic*/)); // Duplicated from Lightdata due to recreating the light colour
				indirectColor = lerp(indirectColor, dot(indirectColor, float3(0.299, 0.587, 0.114)), (0.0 /*_LightingMonochromatic*/)); // Ditto^
				...
				poiLight.rampedLightMap = saturate((poiLight.lightMap - (1 - (1.0 /*_LightingGradientEnd*/))) / saturate((1 - (0.5 /*_LightingGradientStart*/)) - (1 - (1.0 /*_LightingGradientEnd*/)) + fwidth(poiLight.lightMap)));
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, (0.0 /*_LightingIgnoreAmbientColor*/))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), (1.0 /*_ShadowStrength*/));
				directColor *= saturate(poiLight.rampedLightMap + 1 - (1.0 /*_ShadowStrength*/)) * float4(1,1,1,1);
				float3 finalWrap = directColor + indirectColor;
				if ((1.0 /*_LightingCapEnabled*/))
				{
					finalWrap = clamp(finalWrap, (0.0 /*_LightingMinLightBrightness*/), (2.5 /*_LightingCap*/));
				}
				else
				{
					finalWrap = max(finalWrap, (0.0 /*_LightingMinLightBrightness*/));
				}
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - (1.0 /*_ShadowStrength*/));
				#endif
			...
			void applySSAO(float ssao, in PoiMesh poiMesh, inout PoiFragData poiFragData, inout PoiMods poiMods, inout PoiLight poiLight)
			{
				...
				float3 ssaoColor = poiThemeColor(poiMods, float4(0.4338796,0.4338796,0.4338796,1).rgb, (0.0 /*_SSAOColorThemeIndex*/)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_SSAOUseSurfaceColor*/));
				if (any(float4(0,0,0,0)))
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, lilToneCorrection(poiFragData.baseColor, float4(0,0,0,0) + 1), (1 - ssao) * (2.5 /*_SSAOIntensity*/));
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if ((0.0 /*_LightingColorMode*/) == 0) // Poi Custom Light Color
				{
					...
					poiLight.indirectColor = BetterSH9(float4(lerp(0, poiMesh.normals[1], (1.0 /*_LightingIndirectUsesNormals*/)), 1));
				}
				...
				poiLight.attenuationStrength = (0.2 /*_LightingAdditiveCastedShadows*/);
