Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/3376ba1da5060904fad953f4af5802ee"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyBackFaceColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				{
					float4 backFaceColor = float4(0.09084166,0.05087608,0.2140411,0.7490196);
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if ((1.0 /*_LightingAdditiveType*/) == 1)
				{
					...
					float passthrough = 0;
					#else
					float passthrough = (0.05 /*_LightingAdditivePassthrough*/);
					#endif
					float2 ToonAddGradient = float2((0.333 /*_LightingAdditiveGradientStart*/), (0.334 /*_LightingAdditiveGradientEnd*/));
				...
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
				float2 shadowShift0 = float2(float4(-0.01,1,0,1).x, float4(-0.01,1,0,1).y);
				float2 shadowShift1 = float2(float4(-0.01,1,0,1).z, float4(-0.01,1,0,1).w);
				...
				{
					lns.x = poiEdgeNonLinearNoSaturate(lightMap.x, (0.2 /*_ShadowBorder*/), (0.0 /*_ShadowBlur*/) * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, (0.2 /*_ShadowBorder*/), (0.0 /*_ShadowBlur*/) * blurMap.r, (1.0 /*_ShadowBorderRange*/));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, (0.2 /*_ShadowBorder*/), (0.0 /*_ShadowBlur*/) * blurMap.r);
					...
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, (0.2 /*_ShadowBorder*/), (0.0 /*_ShadowBlur*/) * blurMap.r, (1.0 /*_ShadowBorderRange*/));
				}
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (float4(0.7335098,0.7335098,0.791298,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.7335098,0.7335098,0.791298,1).rgb;
				}
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(0.2094424,0.2094424,0.06050144,1).rgb * float4(0.2094424,0.2094424,0.06050144,1).a);
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, (0.0 /*_LightingIgnoreAmbientColor*/));
				#ifndef POI_PASS_ADD
				indirectColor = lerp(indirectColor, poiLight.directColor, poiLight.indirectColor * (0.333 /*_ShadowEnvStrength*/));
				#endif
				...
				if (poiFragData.toggleVertexLights)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (lightingMode == 1)
						{
							float2 ToonAddGradient = float2((0.333 /*_LightingAdditiveGradientStart*/), (0.334 /*_LightingAdditiveGradientEnd*/));
							if (ToonAddGradient.x == ToonAddGradient.y) ToonAddGradient.y += 0.0001;
							vertexLighting = max(vertexLighting, lerp(poiLight.vColor[index], poiLight.vColor[index] * (0.05 /*_LightingAdditivePassthrough*/), smoothstep(ToonAddGradient.x, ToonAddGradient.y, 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow);
						}
					}
			...
			void CalculateUTSSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, (0.2 /*_HighColor_Power*/)))) * (1.0 /*_Layer1Strength*/);
					specMask2 += pow(specArea, exp2(lerp(11, 1, (0.0 /*_Layer2Size*/)))) * (0.0 /*_Layer2Strength*/);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow((0.2 /*_HighColor_Power*/), 5)), (0.0 /*_StylizedSpecularFeather*/)) * (1.0 /*_Layer1Strength*/);
				...
				switch((0.0 /*_Is_BlendAddToHiColor*/))
				{
					case 0:
				...
				if (poiFragData.toggleVertexLights)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if ((0.0 /*_Is_SpecularToHighColor*/))
						{
							vSpecMask = pow(specArea, exp2(lerp(11, 1, (0.2 /*_HighColor_Power*/)))) * (1.0 /*_Layer1Strength*/) * poiLight.vAttenuation[index];
							vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, (0.0 /*_Layer2Size*/)))) * (0.0 /*_Layer2Strength*/) * poiLight.vAttenuation[index]);
						}
						else
						{
							vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow((0.2 /*_HighColor_Power*/), 5)), (0.0 /*_StylizedSpecularFeather*/)) * (1.0 /*_Layer1Strength*/) * poiLight.vAttenuation[index];
			...
			float3 lilCalcSpecular(PoiMesh poiMesh, PoiCam poiCam, float3 L, float3 specular, float attenuation, float roughness)
			{
				...
				return poiEdgeLinear(pow(nh, 1.0 / roughness), (0.1 /*_SpecularBorder*/), (0.0 /*_SpecularBlur*/));
			...
			void lilReflection(inout PoiFragData poiFragData, PoiCam poiCam, PoiLight poiLight, PoiMesh poiMesh, PoiMods poiMods)
			{
				...
				if ((1.0 /*_UseReflection*/) && (1.0 /*_ApplySpecular*/) && (1.0 /*_ApplySpecularFA*/))
				#else
				if ((1.0 /*_UseReflection*/))
				#endif
				{
					float3 reflectCol = 0;
					#if defined(POI_PASS_BASE) || defined(POI_PASS_ADD)
					smoothness = (0.666 /*_Smoothness*/);
					#if defined(PROP_SMOOTHNESSTEX) || !defined(OPTIMIZER_ENABLED)
					...
					float4 reflectionColor = float4(0.01341174,0.01341174,0.01341174,1);
					#if defined(PROP_REFLECTIONCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					if ((1.0 /*_ApplyReflection*/))
					{
						...
						if (!SceneHasReflections() || (0.0 /*_ReflectionCubeOverride*/))
						{
							...
							#endif
							envReflectionColor *= float4(1,1,1,1).rgb * lerp(1.0, poiLight.directColor, (1.0 /*_ReflectionCubeEnableLighting*/));
						}
						else
						{
			...
			{
				float3 backlightColor = float4(0.5225216,0.5225216,0.5225216,1).rgb;
				#if defined(PROP_BACKLIGHTCOLORTEX) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				backlightLN = poiEdgeLinear(backlightLN, (0.47 /*_BacklightBorder*/), (0.0 /*_BacklightBlur*/));
			...
			float CalculateAmbientOcclusion(float2 uv, float depth, float3 normal, float radius, float4 worldDirection, PoiMesh poiMesh, PoiCam poiCam)
			{
				...
				int totalSamples = int(((5.16 /*_SSAOQuality*/) * 5) * smoothstep((8.0 /*_SSAOFalloffEnd*/), (6.0 /*_SSAOFalloffStart*/), poiCam.clipPos.w));
				...
				float totalWeight = 0;
				float randomValue = (0.1 /*_SSAORandomScale*/) * (SSAOInterleavedGradientNoise(uv * _ScreenParams.xy) * 2.0 - 1.0);
				[loop]
				for (int sampleIndex = 0; sampleIndex < totalSamples; sampleIndex++)
				{
					...
					sampleDir = normalize(lerp(sampleDir, normal, (0.4 /*_SSAOCone*/)));
					...
					float depthDifference = depth - sampleDepth - (0.0 /*_SSAOBias*/);
					float weight = 1;
					aoValue = saturate(depthDifference / radius);
					aoValue *= smoothstep(max(radius + float4(0,0.065,1,1).x + EPSILON, radius + float4(0,0.065,1,1).y), radius + float4(0,0.065,1,1).x, depthDifference);
					ao += aoValue;
				}
			...
			float calculateSSAO(PoiMesh poiMesh, PoiCam poiCam, inout PoiLight poiLight, inout PoiMods poiMods)
			{
				...
				float ssao = CalculateAmbientOcclusion(screenPos, depth, transformedNormal, (0.0065 /*_SSAORadius*/), direction, poiMesh, poiCam);
			...
			void applySSAO(float ssao, in PoiMesh poiMesh, inout PoiFragData poiFragData, inout PoiMods poiMods, inout PoiLight poiLight)
			{
				...
				float3 ssaoColor = poiThemeColor(poiMods, float4(0.6038275,0.6038275,0.6038275,1).rgb, (0.0 /*_SSAOColorThemeIndex*/)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_SSAOUseSurfaceColor*/));
				if (any(float4(0,0,0,1)))
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, lilToneCorrection(poiFragData.baseColor, float4(0,0,0,1) + 1), (1 - ssao) * (2.5 /*_SSAOIntensity*/));
				}
				poiFragData.baseColor = lerp(poiFragData.baseColor, customBlend(poiFragData.baseColor, ssaoColor, (2.0 /*_SSAOBlendType*/)), (1 - ssao) * (2.5 /*_SSAOIntensity*/));
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, (0.05 /*_LightingAdditivePassthrough*/));
				...
				#endif
				#ifdef POI_STYLIZED_StylizedSpecular
				if ((1.0 /*_StylizedReflectionMode*/) == 0)
				{
				...
				#ifdef POI_STYLIZED_StylizedSpecular
				if ((1.0 /*_StylizedReflectionMode*/) == 1)
				{
