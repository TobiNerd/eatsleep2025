Shader "Hidden/Locked/.poiyomi/Poiyomi Toon Grab Pass/a4f86e8a74171954fbc15b5ee578dfb7"
{
	Properties
	{
 ...
 #define POI_DEPTHBULGE 
 #define POI_MATCAP0 
 #define POI_VERTEX_GLITCHING 
 #define POI_VERTEX_GLITCHING_TEXTURE 
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDepthBulgeFX(inout VertexOut o)
			{
				...
				#if defined(PROP_DEPTHBULGEMASK) || !defined(OPTIMIZER_ENABLED)
				float depthBulgeMask = tex2Dlod(_DepthBulgeMask, float4(poiUV(vertexUV(o, (0.0 /*_DepthBulgeMaskUV*/)), float4(1,1,0,0)), 0, 0))[(0.0 /*_DepthBulgeMaskChannel*/)];
				#else
				...
				if (depth != 1)
				{
					...
					if (diff > 0)
					{
						intersect = 1 - smoothstep(0, _ProjectionParams.w * (0.01 /*_DepthBulgeFadeLength*/), diff);
					}
				}
				float4 offset = intersect * (0.02 /*_DepthBulgeHeight*/) * float4(o.normal, 0);
			...
			)
			{
				...
				bool canGlitch = true;
				if ((0.0 /*_VertexGlitchMirrorEnable*/) && (0.0 /*_VertexGlitchMirror*/) > 0)
				{
					bool inMirror = IsInMirror();
					if ((0.0 /*_VertexGlitchMirror*/) == 1 && !inMirror)	canGlitch = false;
					if ((0.0 /*_VertexGlitchMirror*/) == 2 && inMirror)	canGlitch = false;
				}
				if (canGlitch)
				{
					...
					#if defined(POI_VERTEX_GLITCHING_TEXTURE)
					float uvl = o.worldPos.y * (2.0 /*_VertexGlitchDensity*/) + _Time.x * (3.0 /*_VertexGlitchMapPanSpeed*/);
					float uvr = o.worldPos.y * (2.0 /*_VertexGlitchDensity*/) - _Time.x * (3.0 /*_VertexGlitchMapPanSpeed*/);
					...
					#endif
					float time = _Time.y * (0.0 /*_VertexGlitchFrequency*/);
					float randomGlitch = (sin(time) + sin(2.2 * time + 5.52) + sin(2.9 * time + 0.93) + sin(4.6 * time + 8.94)) / 4;
					float3 glitchOffset = 0;
					#ifdef POI_AUDIOLINK
					if (AudioLinkIsAvailable() && (0.0 /*_VertexGlitchingAudioLinkEnabled*/))
					{
						float audioIntensity =
						AudioLinkData(ALPASS_AUDIOBASS).r 		* ((0.0 /*_VertexGlitchingAudioLinkBand*/) == 0) +
						AudioLinkData(ALPASS_AUDIOLOWMIDS).r 	* ((0.0 /*_VertexGlitchingAudioLinkBand*/) == 1) +
						AudioLinkData(ALPASS_AUDIOHIGHMIDS).r	* ((0.0 /*_VertexGlitchingAudioLinkBand*/) == 2) +
						AudioLinkData(ALPASS_AUDIOTREBLE).r 	* ((0.0 /*_VertexGlitchingAudioLinkBand*/) == 3) +
						AudioLinkData(ALPASS_FILTEREDVU_INTENSITY).r * ((0.0 /*_VertexGlitchingAudioLinkBand*/) == 4);
						if((1.0 /*_VertexGlitchingAudiolinkOverride*/))
						{
							glitchOffset = glitchAmount * glitchDirection * ((0.04 /*_VertexGlitchStrength*/) * .01) * audioIntensity;
						} else {
							glitchOffset = glitchAmount * glitchDirection * ((0.04 /*_VertexGlitchStrength*/) * .01) * step((0.0 /*_VertexGlitchThreshold*/), randomGlitch);
							glitchOffset += glitchAmount * glitchDirection * ((0.04 /*_VertexGlitchStrength*/) * .01) * audioIntensity;
						}
					} else {
						glitchOffset = glitchAmount * glitchDirection * ((0.04 /*_VertexGlitchStrength*/) * .01) * step((0.0 /*_VertexGlitchThreshold*/), randomGlitch);
			...
			void applyAlphaOptions(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				{
					float holoRim = saturate(1 - smoothstep(min((0.0 /*_AlphaFresnelSharpness*/), (0.7 /*_AlphaFresnelWidth*/)), (0.7 /*_AlphaFresnelWidth*/), (poiCam.vDotN)));
					holoRim = abs(lerp(1, holoRim, (0.5 /*_AlphaFresnelAlpha*/)));
					holoRim = (1.0 /*_AlphaFresnelInvert*/) ? 1 - holoRim : holoRim;
			...
			float4 grabpassBlur(float2 uv, float4 grabPos, float smoothness)
			{
				...
				float2 radius = ((0.0 /*_GrabBlurDistance*/) + smoothness) * .1;
				...
				for (int i = 0; i < directions; i++)
				{
					for (int j = 0; j < quality; j++)
					{
						...
						float offset = pow(baseOffset, (0.0 /*_GrabBlurPower*/) + 1); // Apply flexible power
			...
			void applyGrabEffects(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					grabpassColor = Refraction((1.35 /*_RefractionIndex*/), (0.0 /*_RefractionChromaticAberattion*/), poiMesh, poiCam, poiMods, poiLight, poiFragData).rgb;
				}
				else
				{
				...
				grabpassColor = (float4(1,1,1,1) * grabpassColor * (1.0 /*_GrabPassBrightness*/)) + ((0.0 /*_GrabPassAddBrightness*/) * float4(1,1,1,1));
				#ifdef POI_PASS_ADD
			...
			void calculateGlobalThemes(inout PoiMods poiMods)
			{
				...
				DecomposeHDRColor(float4(0.1650943,0.5678136,1,1).rgb, themeColor0.rgb, themeColorExposures.x);
				...
				poiMods.globalColorTheme[0] = float4(ApplyHDRExposure(ModifyViaHSV(themeColor0.rgb, frac((0.0 /*_GlobalThemeHue0*/) + (0.0 /*_GlobalThemeHueSpeed0*/) * _Time.x), (0.0 /*_GlobalThemeSaturation0*/), (0.0 /*_GlobalThemeValue0*/)), themeColorExposures.x), float4(0.1650943,0.5678136,1,1).a);
			...
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				...
				#if defined(PROP_DETAILNORMALMAP) || !defined(OPTIMIZER_ENABLED)
				half3 detailNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_DetailNormalMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DetailNormalMapUV*/)], float4(6,6,0,0)), float4(-0.5,-0.5,0,0), (0.0 /*_DetailNormalMapStochastic*/)), (0.15 /*_DetailNormalMapScale*/) * poiMods.detailMask.g);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					matcap.rgb = hueShift(matcap.rgb, (0.0 /*_MatcapHueShift*/) + _Time.x * (0.0 /*_MatcapHueShiftSpeed*/), (0.0 /*_MatcapHueShiftColorSpace*/), (1.0 /*_MatcapHueSelectOrShift*/));
				}
				#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
				...
				blendMatcap(poiLight, poiFragData, poiMods, (0.067 /*_MatcapAdd*/), (0.0 /*_MatcapAddToLight*/), (0.0 /*_MatcapMultiply*/), (0.0 /*_MatcapReplace*/), (0.0 /*_MatcapMixed*/), (0.0 /*_MatcapScreen*/), matcap, matcapMask, (0.0 /*_MatcapEmissionStrength*/), (0.584 /*_MatcapLightMask*/), (0.0 /*_MatcapMaskGlobalMask*/), (2.0 /*_MatcapMaskGlobalMaskBlendType*/), matcapALD);
				#endif
			}
			#endif
			...
			{
				float smoothness = (0.9 /*_MochieRoughnessMultiplier*/);
			...
			{
				float clearCoatMask = (0.1 /*_ClearCoatStrength*/);
				float smoothness = (0.946 /*_ClearCoatSmoothness*/);
				...
				if ((0.0 /*_ClearCoatReflectionStrengthGlobalMask*/) > 0)
				{
					reflectionMask = customBlend(reflectionMask, poiMods.globalMask[(0.0 /*_ClearCoatReflectionStrengthGlobalMask*/) - 1], (2.0 /*_ClearCoatReflectionStrengthGlobalMaskBlendType*/));
				}
				if ((0.0 /*_ClearCoatSpecularStrengthGlobalMask*/) > 0)
				{
					specularMask = customBlend(specularMask, poiMods.globalMask[(0.0 /*_ClearCoatSpecularStrengthGlobalMask*/) - 1], (2.0 /*_ClearCoatSpecularStrengthGlobalMaskBlendType*/));
				}
				...
				float surfaceReduction = (1.0 / (brdfRoughness * brdfRoughness + 1.0));
				float grazingTerm = saturate(smoothness + (1 - omr));
				float3 reflCol = GetReflections(poiCam, poiLight, poiMesh, roughness, (0.0 /*_ClearCoatForceFallback*/), (0.0 /*_ClearCoatLitFallback*/), _ClearCoatFallback, _ClearCoatFallback_HDR, clearcoatReflectionDir);
			...
			void applyDepthFX(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_DEPTHMASK) || !defined(OPTIMIZER_ENABLED)
				float depthMask = POI2D_SAMPLER_PAN(_DepthMask, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DepthMaskUV*/)], float4(1,1,0,0)), float4(0,0,0,0))[(0.0 /*_DepthMaskChannel*/)];
				#else
				float depthMask = 1;
				#endif
				if ((0.0 /*_DepthMaskGlobalMask*/) > 0)
				{
					depthMask = maskBlend(depthMask, poiMods.globalMask[(0.0 /*_DepthMaskGlobalMask*/) - 1], (2.0 /*_DepthMaskGlobalMaskBlendType*/));
				}
				if ((1.0 /*_DepthColorToggle*/))
				{
					float colorBlendAlpha = lerp((0.0 /*_DepthColorMinValue*/), (0.2 /*_DepthColorMaxValue*/), remapClamped((0.05 /*_DepthColorMinDepth*/), (0.0 /*_DepthColorMaxDepth*/), diff));
					#if defined(PROP_DEPTHTEXTURE) || !defined(OPTIMIZER_ENABLED)
					float2 depthTextureUV = float2(0, 0);
					if ((0.0 /*_DepthTextureUV*/) == 8)
					{
						depthTextureUV = lerp(0, 1, remapClamped((0.05 /*_DepthColorMinDepth*/), (0.0 /*_DepthColorMaxDepth*/), diff));
					}
					else
					{
						depthTextureUV = poiMesh.uv[(0.0 /*_DepthTextureUV*/)];
					}
					float3 depthColor = POI2D_SAMPLER_PAN(_DepthTexture, _MainTex, poiUV(depthTextureUV, float4(1,1,0,0)), float4(0,0,0,0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1), (1.0 /*_DepthColorThemeIndex*/));
					#else
					float3 depthColor = poiThemeColor(poiMods, float4(1,1,1,1), (1.0 /*_DepthColorThemeIndex*/));
					#endif
					switch((2.0 /*_DepthColorBlendMode*/))
					{
						case 0:
						{
					...
					poiFragData.emission += depthColor * colorBlendAlpha * (0.0 /*_DepthEmissionStrength*/) * depthMask;
				}
				if ((0.0 /*_DepthAlphaToggle*/))
				{
					poiFragData.alpha *= lerp(poiFragData.alpha, saturate(lerp((1.0 /*_DepthAlphaMinValue*/), (0.0 /*_DepthAlphaMaxValue*/), remapClamped((0.0 /*_DepthAlphaMinDepth*/), (0.0 /*_DepthAlphaMaxDepth*/), diff))), depthMask);
				}
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_BumpMapUV*/)].xy, float4(3,3,0,0)), float4(1,0.5,0,0), (0.0 /*_BumpMapStochastic*/)), (0.29 /*_BumpScale*/));
				#else
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(1,1,1,0.3490196).rgb, (1.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(1,1,1,0.3490196).a;
