Shader "Hidden/Locked/.poiyomi/Poiyomi Toon Grab Pass/8d183e97b99f26040a156cbc60955feb"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyAlphaOptions(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				{
					float holoRim = saturate(1 - smoothstep(min((0.067 /*_AlphaFresnelSharpness*/), (0.763 /*_AlphaFresnelWidth*/)), (0.763 /*_AlphaFresnelWidth*/), (poiCam.vDotN)));
					holoRim = abs(lerp(1, holoRim, (0.875 /*_AlphaFresnelAlpha*/)));
			...
			float4 grabpassBlur(float2 uv, float4 grabPos, float smoothness)
			{
				...
				float2 radius = ((0.036 /*_GrabBlurDistance*/) + smoothness) * .1;
				radius.x *= aspect; // Adjust for aspect ratio
				float quality = floor((5.0 /*_GrabBlurQuality*/));
				float directions = floor((5.0 /*_GrabBlurDirections*/));
			...
			inline float4 Refraction(float indexOfRefraction, float chromaticAberration, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, inout PoiLight poiLight, in PoiFragData poiFragData)
			{
				...
				else
				{
					...
					refractionColor = grabpassBlur(refractedGrab, poiCam.posScreenSpace, (1 - poiFragData.smoothness) * (0.5 /*_GrabBlurUseSmoothness*/));
					#else
			...
			void applyGrabEffects(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					grabpassColor = Refraction((1.3 /*_RefractionIndex*/), (0.0 /*_RefractionChromaticAberattion*/), poiMesh, poiCam, poiMods, poiLight, poiFragData).rgb;
				}
				else
				{
					#ifdef CHROMATIC_ABERRATION_LOW
					grabpassColor = grabpassBlur(poiCam.screenUV, poiCam.posScreenSpace, (1 - poiFragData.smoothness) * (0.5 /*_GrabBlurUseSmoothness*/));
					#else
			...
			{
				float smoothness = (0.894 /*_MochieRoughnessMultiplier*/);
				...
				specularMask *= (3.0 /*_MochieSpecularStrength*/);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_MainTex, _MainTex, mainUV, float4(0,0,0,0), (1.0 /*_MainTexStochastic*/));
				mainTexture.a = max(mainTexture.a, (0.0 /*_MainIgnoreTexAlpha*/));
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_BumpMapUV*/)].xy, float4(2,5,0,0)), float4(0,0,0,0), (0.0 /*_BumpMapStochastic*/)), (0.36 /*_BumpScale*/));
				#else
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.08839609,0.471447,0.7986599,0.4313726).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.08839609,0.471447,0.7986599,0.4313726).a;
